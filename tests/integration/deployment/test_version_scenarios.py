...
def get_version_from_build(project_root: Path) -> str:
    ...
    if match := re.match(r"code_weaver[_-]?(.+?)-py3-none-any\.whl", wheel_name):
        return match[1]
...
def test_version_scenarios():
    ...
    if commit_distance == 0 and not is_dirty:
        ...
        assert re.match(r"^\d+\.\d+\.\d+$", version) or re.match(
    ...
    assert re.match(pep440_pattern, version), f"Version must be PEP 440 compliant, got: {version}"
...
def test_version_consistency_across_formats():
    ...
    wheel_match = re.match(r"code_weaver[_-]?(.+?)-py3-none-any\.whl", wheels[0].name)
    sdist_match = re.match(r"code_weaver[_-]?(.+?)\.tar\.gz", sdists[0].name)

...
@pytest.mark.parametrize(
    ...
    [
        ...
        (".JSON", ConfigLanguage.JSON),
        ("MAKEFILE", None),  # Case matters for exact filename matches in ConfigLanguage (e.g. Makefile)
        (".unknown", None),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (".py", SemanticSearchLanguage.PYTHON),
        (".js", SemanticSearchLanguage.JAVASCRIPT),
        ("py", None), # has_semantic_extension expects actual dots, the from_extension maps logic handles string variations.
        (".PY", None), # has_semantic_extension checks exact list match and since extensions in map are lowercased it fails unless normalized.
        (".unknown", None),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("MAKEFILE", None), # Testing case variants (Makefile relies on case sensitive matching or specific extension)
        ("unknown.file", None),
    ],
)

...
async def test_memory_persistence_performance(chunk_count: int, vector_store_factory) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # WSL2 JSON serialization of 10k chunks consistently takes 30-35s on this machine.
        if chunk_count == 10000:
            assert 0.5 <= persist_duration <= 45.0, (
                f"Persist 10k chunks took {persist_duration:.3f}s, outside 0.5-45s relaxed range"
            )
            assert restore_duration <= 45.0, (
                f"Restore 10k chunks took {restore_duration:.3f}s, should be under 45s"
            )

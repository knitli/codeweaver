...
#
# * This is the function that gets called when an MCP agent invokes the `find_code` tool. The mcp Tool definition is in `codeweaver.server.mcp.tools`. This find_code_tool wraps the actual `find_code`, which is defined in `codeweaver.agent_api`.
# -------------------------

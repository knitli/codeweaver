...
async def stats_info(_request: Request) -> PlainTextResponse:
    ...
    if (stats := statistics()):
        try:
            ...
            return PlainTextResponse(content=to_json({'error': f'Failed to serialize session statistics: {e}'}), status_code=500, media_type='application/json')
    return PlainTextResponse(content=to_json({'error': 'No metrics available'}), status_code=500, media_type='application/json')
...
async def settings_info(_request: Request) -> PlainTextResponse:
    ...
    try:
        return PlainTextResponse(content=to_json(dict(settings_view.items())), media_type='application/json')
    except Exception as e:
        logger.exception('Failed to serialize settings')
        return PlainTextResponse(content=to_json({'error': f'Failed to serialize settings: {e}'}), status_code=500, media_type='application/json')
...
async def version_info(_request: Request) -> PlainTextResponse:
    ...
    try:
        return PlainTextResponse(content=to_json({'version': __version__}), media_type='application/json')
    except Exception as e:
        logger.exception('Failed to serialize version information')
        return PlainTextResponse(content=to_json({'error': f'Failed to serialize version information: {e}'}), status_code=500, media_type='application/json')
...
async def health(_request: Request) -> PlainTextResponse:
    ...
    try:
        ...
        if state.health_service is None:
            ...
            from codeweaver.server.health.models import EmbeddingProviderServiceInfo, HealthResponse, IndexingInfo, IndexingProgressInfo, RerankingServiceInfo, ServicesInfo, SparseEmbeddingServiceInfo, StatisticsInfo, VectorStoreServiceInfo
            error_response = HealthResponse.create_with_current_timestamp(status='unhealthy', uptime_seconds=0, indexing=IndexingInfo(state='error', last_indexed=None, progress=IndexingProgressInfo(files_discovered=0, files_processed=0, chunks_created=0, errors=0, current_file=None, start_time=None, estimated_completion=None)), services=ServicesInfo(vector_store=VectorStoreServiceInfo(status='down', latency_ms=0), embedding_provider=EmbeddingProviderServiceInfo(status='down', model='unknown', latency_ms=0, circuit_breaker_state='open'), sparse_embedding=SparseEmbeddingServiceInfo(status='down', provider='unknown'), reranking=RerankingServiceInfo(status='down', model='unknown', latency_ms=0)), statistics=StatisticsInfo(total_chunks_indexed=0, total_files_indexed=0, languages_indexed=[], index_size_mb=0, queries_processed=0, avg_query_latency_ms=0, semantic_chunks=0, delimiter_chunks=0, file_chunks=0, avg_chunk_size=0))
            return PlainTextResponse(content=error_response.model_dump_json(), status_code=503, media_type='application/json')
        ...
        return PlainTextResponse(content=health_response.model_dump_json(), media_type='application/json')
...
async def favicon(_request: Request) -> Response:
    ...
    return Response(content=svg_data, media_type='image/svg+xml', headers={'Cache-Control': 'public, max-age=259200'})
...
async def status_info(_request: Request) -> PlainTextResponse:
    ...
    try:
        state = get_state()
        status_data: dict[str, Any] = {'timestamp': datetime.now(UTC).isoformat(), 'uptime_seconds': int(time.time() - state.startup_time) if hasattr(state, 'startup_time') else 0}
        if state.indexer:
            indexer_stats = state.indexer.stats
            status_data['indexing'] = {'active': state.indexer._running if hasattr(state.indexer, '_running') else False, **indexer_stats.dump_python()}
        else:
        ...
        if (stats := statistics()):
            status_data['statistics'] = {'total_requests': stats.total_requests, 'successful_requests': stats.successful_requests, 'failed_requests': stats.failed_requests}
    except Exception:
        logger.exception('Failed to get status information')
        return PlainTextResponse(content=to_json({'error': 'Failed to retrieve status information'}), status_code=500, media_type='application/json')
    else:
...
async def shutdown_handler(request: Request) -> PlainTextResponse:
    ...
    if request.method != 'POST':
        return PlainTextResponse(content=to_json({'error': 'Method not allowed. Use POST.'}), status_code=405, media_type='application/json')
    if not ManagementServer.request_shutdown():
        return PlainTextResponse(content=to_json({'error': 'Shutdown could not be initiated'}), status_code=500, media_type='application/json')
    logger.info('Shutdown requested via management API')
    return PlainTextResponse(content=to_json({'status': 'shutdown_initiated', 'message': 'Daemon shutdown initiated'}), status_code=202, media_type='application/json')
...
class ManagementServer:
    ...
    def create_app(self) -> Starlette:
        ...
        routes = [Route('/favicon.ico', favicon, methods=['GET'], include_in_schema=False), Route('/health', health, methods=['GET']), Route('/status', status_info, methods=['GET']), Route('/metrics', stats_info, methods=['GET']), Route('/shutdown', shutdown_handler, methods=['POST'])]

...
try:
    ...
    raise ConfigurationError('Please install the `voyageai` package to use the Voyage provider, you can use the `voyage` optional group -- `pip install "code-weaver\\[voyage]"`') from _import_error

def voyage_context_output_transformer(result: ContextualizedEmbeddingsObject) -> list[list[float]] | list[list[int]]:
...
class VoyageEmbeddingProvider(EmbeddingProvider[AsyncClient]):
    ...
    _output_transformer: ClassVar[Callable[[Any], list[list[float]] | list[list[int]]]] = voyage_output_transformer
    ...
    def _initialize(self, caps: EmbeddingModelCapabilities) -> None:
        ...
        shared_kwargs = {'model': caps.name, 'output_dimension': configured_dimension, 'output_dtype': 'float'}
    ...
    def _process_output(self, output_data: Any) -> list[list[float]] | list[list[int]]:
        ...
        transformer = voyage_context_output_transformer if self._is_context_model else voyage_output_transformer
    ...
    async def _embed_documents(self, documents: Sequence[CodeChunk], **kwargs: Any) -> list[list[float]] | list[list[int]]:
        """Embed a list of documents into vectors."""
        import logging
        logger = logging.getLogger(__name__)
        ready_documents = cast(list[str], self.chunks_to_strings(documents))
        try:
            results: EmbeddingsObject = await self.client.embed(texts=ready_documents, **kwargs | self.doc_kwargs)
            ...
            if 'max allowed tokens per submitted batch' in error_msg.lower() and len(documents) > 1:
                logger.warning('Voyage batch token limit exceeded (%s), splitting batch of %d chunks in half and retrying', error_msg.split('Your batch has')[1].split('tokens')[0].strip() if 'Your batch has' in error_msg else 'unknown', len(documents))
                mid = len(documents) // 2
    ...
    async def _embed_query(self, query: Sequence[str], **kwargs: Any) -> list[list[float]] | list[list[int]]:
        """Embed a query or group of queries into vectors."""
        results: EmbeddingsObject = await self.client.embed(texts=list(query), **kwargs | self.query_kwargs)
        self._fire_and_forget(lambda: self._update_token_stats(token_count=results.total_tokens))
        return self._process_output(results)
    ...
    def dimension(self) -> int:
        ...
        return self.doc_kwargs.get('output_dimension', self.caps.default_dimension)

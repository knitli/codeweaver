...
class XAIClientOptions(ClientOptions):
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('api_key', 'management_api_key', 'channel_options')} | {FilteredKey(name): AnonymityConversion.HASH for name in ('api_host', 'management_api_host') if not getattr(self, name).endswith('x.ai')}
...
class BaseAnthropicClientOptions(ClientOptions):
    ...
    _core_provider: ClassVar[Literal[Provider.ANTHROPIC, Provider.BEDROCK, Provider.AZURE, Provider.GOOGLE, Provider.GROQ]]
    _providers: ClassVar[tuple[Provider, ...]] = (Provider.ANTHROPIC, Provider.BEDROCK, Provider.AZURE, Provider.GOOGLE, Provider.GROQ)
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('http_client', 'default_headers', 'default_query')} | {FilteredKey('base_url'): AnonymityConversion.HASH if self.base_url and _test_url_for_provider(str(self.base_url), 'anthropic.com') else AnonymityConversion.BOOLEAN}
...
class AnthropicClientOptions(BaseAnthropicClientOptions):
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('api_key', 'auth_token')} | super()._telemetry_keys()
...
class AnthropicBedrockClientOptions(BaseAnthropicClientOptions):
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('aws_secret_key', 'aws_access_key', 'aws_session_token')} | {FilteredKey('aws_region'): AnonymityConversion.HASH, FilteredKey('aws_profile'): AnonymityConversion.HASH} | super()._telemetry_keys()
...
class AnthropicAzureClientOptions(BaseAnthropicClientOptions):
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('api_key', 'azure_ad_token_provider')} | {FilteredKey('resource'): AnonymityConversion.HASH} | super()._telemetry_keys()
...
class AnthropicGoogleVertexClientOptions(BaseAnthropicClientOptions):
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('credentials', 'access_token')} | {FilteredKey('project_id'): AnonymityConversion.HASH} | super()._telemetry_keys()
...
class OpenAIAgentClientOptions(OpenAIClientOptions):
    ...
    _providers: ClassVar[tuple[Provider, ...]] = tuple(provider for provider in Provider if provider.uses_openai_api and provider != Provider.GROQ)
    ...
    def computed_base_url(self, provider: Provider) -> str | None:
        ...
        provider: Provider = provider if isinstance(provider, Provider) else Provider.from_string(provider)
        if (found_provider_url := super().computed_base_url(provider)):
            return found_provider_url
        if (found_agent_provider_url := {Provider.ALIBABA: 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1', Provider.CEREBRAS: 'https://api.cerebras.ai/v1', Provider.DEEPSEEK: 'https://api.deepseek.com/v1', Provider.FIREWORKS: 'https://api.fireworks.ai/inference/v1', Provider.GITHUB: 'https://models.github.ai/inference/v1', Provider.LITELLM: 'http://0.0.0.0:4000', Provider.MOONSHOT: 'https://api.moonshot.ai/v1', Provider.NEBIUS: 'https://api.studio.nebius.com/v1', Provider.OPENROUTER: 'https://openrouter.ai/api/v1', Provider.OVHCLOUD: 'https://oai.endpoints.kepler.ai.cloud.ovh.net/v1', Provider.SAMBANOVA: 'https://api.sambanova.ai/v1'}.get(cast(Provider, provider))):
...
class PydanticGatewayClientOptions(ClientOptions):
    ...
    upstream_provider: Literal['openai', 'groq', 'anthropic', 'bedrock', 'gemini', 'google-vertex'] | LiteralStringT
    ...
    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey(name): AnonymityConversion.BOOLEAN for name in ('api_key', 'http_client', 'default_headers', 'default_query')} | ({} if 'pydantic' in str(self.base_url) else {FilteredKey('base_url'): AnonymityConversion.HASH})
...
def discriminate_anthropic_agent_client_options(v: Any) -> str:
    ...
    if (base_url := str(v.get('base_url') if isinstance(v, dict) else getattr(v, 'base_url', None))):
        ...
        if _test_url_for_provider(base_url, 'googleapis.com'):
            return 'anthropic-google'
    return 'anthropic'
type AnthropicAgentClientOptionsType = Annotated[Annotated[AnthropicClientOptions, Tag(Provider.ANTHROPIC.variable)] | Annotated[AnthropicBedrockClientOptions, Tag(Provider.BEDROCK.variable)] | Annotated[AnthropicAzureClientOptions, Tag(Provider.AZURE.variable)] | Annotated[AnthropicGoogleVertexClientOptions, Tag(Provider.GOOGLE.variable)], Field(description='Anthropic agent client options type.', discriminator='tag')]
type SimpleAgentClientOptionsType = Annotated[Annotated[CohereClientOptions, Tag(Provider.COHERE.variable)] | Annotated[BedrockClientOptions, Tag(Provider.BEDROCK.variable)] | Annotated[OpenAIClientOptions, Tag(Provider.OPENAI.variable)] | Annotated[GoogleClientOptions, Tag(Provider.GOOGLE.variable)] | Annotated[GroqClientOptions, Tag(Provider.GROQ.variable)] | Annotated[HuggingFaceClientOptions, Tag(Provider.HUGGINGFACE_INFERENCE.variable)] | Annotated[MistralClientOptions, Tag(Provider.MISTRAL.variable)] | Annotated[PydanticGatewayClientOptions, Tag(Provider.PYDANTIC_GATEWAY.variable)] | Annotated[XAIClientOptions, Tag(Provider.X_AI.variable)], Field(description='Agent client options type.', discriminator=Discriminator(lambda v: simple_provider_discriminator(v) if simple_provider_discriminator(v) in {'cohere', 'openai', 'google', 'hf_inference', 'mistral', 'gateway', 'x_ai', 'groq', 'bedrock'} else 'openai'))]
type GeneralAgentClientOptionsType = Annotated[Annotated[AnthropicAgentClientOptionsType, Tag('anthropic')] | Annotated[SimpleAgentClientOptionsType, Tag('other')], Field(description='General agent client options type.', discriminator=ANTHROPIC_CLIENT_OPTIONS_AGENT_DISCRIMINATOR)]
__all__ = ('AnthropicAgentClientOptionsType', 'AnthropicAzureClientOptions', 'AnthropicBedrockClientOptions', 'AnthropicClientOptions', 'AnthropicGoogleVertexClientOptions', 'BaseAnthropicClientOptions', 'GeneralAgentClientOptionsType', 'GroqClientOptions', 'OpenAIAgentClientOptions', 'PydanticGatewayClientOptions', 'SimpleAgentClientOptionsType', 'XAIClientOptions', 'discriminate_anthropic_agent_client_options')

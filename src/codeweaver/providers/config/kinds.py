...
type LiteralSDKClient = Literal[SDKClient.BEDROCK, SDKClient.COHERE, SDKClient.FASTEMBED, SDKClient.OPENAI, SDKClient.SENTENCE_TRANSFORMERS, SDKClient.QDRANT]
...
class ConnectionConfiguration(BasedModel):
    ...
    headers: Annotated[dict[str, str] | None, Field(description='HTTP headers to include in requests.')] = None
    rate_limits: Annotated[ConnectionRateLimitConfig | None, Field(description='Rate limit configuration for the connection.')] = None
    httpx_config: Annotated[HttpxClientParams | None, Field(description="You may optionally provide custom client parameters for the httpx client. CodeWeaver will use your parameters when it constructs its http client pool. You probably don't need this unless you need to handle unique auth or similar requirements.")] = None

    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey('headers'): AnonymityConversion.BOOLEAN, FilteredKey('httpx_config'): AnonymityConversion.BOOLEAN}

class BaseProviderSettings(BasedModel, ABC):
    """Base settings for all providers."""
    provider: Provider
    connection: ConnectionConfiguration | None = None
    client_options: Annotated[ClientOptions | None, Field(description="Client options for the provider's client.")] = None
    ...
    def __init__(self, **data: Any) -> None:
        ...
        try:
            ...
            logger.debug('Dependency injection container not available, skipping registration of ProviderSettings: %s', e)
        ...
        if 'model_name' in type(self).model_fields and 'model_name' not in data and (kind_config := (getattr(self, 'embedding_config', None) or getattr(self, 'sparse_embedding_config', None) or getattr(self, 'reranking_config', None))) and kind_config.model_name:
            object.__setattr__(self, 'model_name', kind_config.model_name)
        elif 'model_name' in type(self).model_fields and 'model_name' not in data and self.client_options and (model_name := next((getattr(self.client_options, k, None) for k in ('model_name', 'model', 'model_id', 'model_name_or_path') if getattr(self.client_options, k, None)), None)):
    ...
    def __model_post_init__(self) -> None:
        ...
        try:
            ...
            logger.debug('Failed to register %s in DI container (monorepo mode): %s', type(self).__name__, e)
    ...
    def _ensure_endpoint_version(self) -> Self:
        ...
        if not self.client_options:
            return self
        if self.client_options and self.client_options._core_provider in {Provider.COHERE, Provider.OPENAI} and (endpoint := getattr(self.client_options, 'base_url', None)):
            object.__setattr__(self, 'client_options', self.client_options.model_copy(update={'base_url': ensure_endpoint_version(endpoint, cohere=self.client_options._core_provider == Provider.COHERE)}))
        return self
    ...
    def get_client(self) -> Any:
        ...
        options = self.client_options.as_settings() if isinstance(self.client_options, ClientOptions) else {}
        client_import = cast(SDKClient, self.client).client
        kind = next((name for name in {'sparse', 'embed', 'rerank'} if name in type(self).__name__.lower()), None)
        if self.provider == Provider.BEDROCK:
            if not kind:
                raise CodeWeaverDeveloperError("Kind must be one of 'sparse', 'embed', or 'rerank' for Bedrock provider. File an issue. This is unexpected.")
            return client_import._resolve()('bedrock-runtime' if kind == 'embed' else 'bedrock-agent-runtime', **options)
...
class AzureProviderMixin:
    ...
    **We only support the "**next-generation** Azure OpenAI API." Currently, you need to opt into this API in your Azure settings. We didn't want to start supporting the old API knowing it's going away.
    ...
    """
    azure_resource_name: Annotated[str, Field(description='The name of your Azure resource. This is used to identify your resource in Azure.')]
    model_deployment: Annotated[str, Field(description='The deployment name of the model you want to use. This is *different* from the model name in `model_options`, which is the name of the model itself (`text-embedding-3-small`). You need to create a deployment in your Azure OpenAI resource for each model you want to use, and provide the deployment name here.')]
    endpoint: Annotated[str | None, Field(description='The endpoint for your Azure resource. This is used to send requests to your resource. Only provide the endpoint, not the full URL. For example, if your endpoint is `https://your-cool-resource.<region_name>.inference.ai.azure.com/v1`, you would only provide "your-cool-resource" here.')] = None
    region_name: Annotated[str | None, Field(description='The region name for your Azure resource. This is used to identify the region your resource is in. For example, `eastus` or `westus2`.')] = None
    api_key: Annotated[SecretStr | None, Field(description="Your Azure API key. If not provided, we'll assume you have your Azure credentials configured in another way, such as environment variables.")] = None

    def _telemetry_keys(self) -> dict[FilteredKeyT, AnonymityConversion]:
        return {FilteredKey('azure_resource_name'): AnonymityConversion.HASH, FilteredKey('model_deployment'): AnonymityConversion.HASH, FilteredKey('endpoint'): AnonymityConversion.HASH, FilteredKey('region_name'): AnonymityConversion.HASH, FilteredKey('api_key'): AnonymityConversion.BOOLEAN}

    def as_azure_options(self) -> AzureOptions:
        """Return the settings as an AzureOptions TypedDict."""
        return AzureOptions(model_deployment=self.model_deployment, endpoint=self.endpoint, region_name=self.region_name, api_key=self.api_key)
...
class VectorStoreProviderSettings(BaseProviderSettings):
    ...
    batch_size: Annotated[PositiveInt | None, Field(description='Batch size for bulk upsert operations. Defaults to 64.')] = 64
...
class CollectionConfig(BasedModel):
    ...
    hnsw_config: HnswConfig | None = Field(default_factory=lambda: HnswConfig(m=24, ef_construct=130, payload_m=120, full_scan_threshold=10000))
    ...
    async def set_vector_params(self, embedding_group: EmbeddingCapabilityGroup | None=None) -> None:
        ...
        self.sparse_vectors_config = _deep_merge(dict(self.sparse_vectors_config or {}), cast(dict[str, SparseVectorParams], params.sparse_vectors or {}))
    ...
    async def as_qdrant_config(self, metadata: CollectionMetadata) -> QdrantCollectionConfig:
        ...
        return QdrantCollectionConfig.model_construct(params=await self.params(), hnsw_config=self.hnsw_config, optimizer_config=self.optimizer_config, wal_config=self.wal_config, metadata=metadata.model_dump())
...
class _BaseQdrantVectorStoreProviderSettings(VectorStoreProviderSettings):
    ...
    client_options: Annotated[QdrantClientOptions | None, Field(description="Client options for the provider's client.")] = None
    collection: Annotated[CollectionConfig, Field(description='Collection configuration for the vector store.')]

    def _initialize(self, **data: Any) -> None:
        """Initialize Qdrant vector store provider settings."""

    def __init__(self, provider: Literal[Provider.QDRANT, Provider.MEMORY]=Provider.QDRANT, connection: ConnectionConfiguration | None=None, client_options: QdrantClientOptions | None=None, collection: CollectionConfig | None=None, batch_size: PositiveInt | None=96, *, project_name: str | None=None, project_path: Path | None=None) -> None:
        """Initialize Qdrant vector store provider settings.
        ...
        prepared_collection = CollectionConfig.model_validate(self._default_collection(project_name=project_name, project_path=project_path).model_dump() | collection_dict)
        constructed = self.__class__.model_construct(provider=provider, tag=provider.variable, connection=connection, client_options=prepared_client_options, collection=prepared_collection, batch_size=batch_size)
    ...
    def _default_collection(self, *, project_name: str | None=None, project_path: Path | None=None) -> CollectionConfig:
        ...
        return CollectionConfig(collection_name=generate_collection_name(project_name=project_name, project_path=project_path), vectors_config=None, sparse_vectors_config=None)

    @model_validator(mode='after')
    def _ensure_consistent_config(self) -> Self:
        """Ensure consistent config for Qdrant and Memory providers."""
        if not self.client_options:
            self.client_options = QdrantClientOptions(location=':memory:' if self.provider == Provider.MEMORY else None, host='localhost' if self.provider == Provider.QDRANT else None)
        if self.provider == Provider.MEMORY:
            ...
            self.in_memory_config = self.in_memory_config or getattr(self, '_default_memory_config', lambda _x: {'collection_name': generate_collection_name()})(collection_name)
        try:
            from codeweaver.core.di import get_container
            container = get_container()
    ...
    async def get_collection_config(self, metadata: CollectionMetadata, *, embedding_group: EmbeddingCapabilityGroup | None=None, failover_settings: FailoverSettings | None=None, failover_detector: FailoverDetector | None=None) -> QdrantCollectionConfig:
...
class MemoryVectorStoreProviderSettings(_BaseQdrantVectorStoreProviderSettings):
    ...
    in_memory_config: Annotated[MemoryConfig, Field(description='In-memory vector store configuration.')]

    def __init__(self, provider: Literal[Provider.MEMORY]=Provider.MEMORY, connection: ConnectionConfiguration | None=None, client_options: QdrantClientOptions | None=None, collection: CollectionConfig | None=None, batch_size: PositiveInt | None=96, in_memory_config: MemoryConfig | None=None, *, project_name: str | None=None, project_path: Path | None=None) -> None:
        ...
        if collection is None:
            ...
            collection_dict = collection
        parent_default_collection = super()._default_collection(project_name=project_name, project_path=project_path)
        prepared_collection = CollectionConfig.model_validate(parent_default_collection.model_dump() | collection_dict)
        collection_name = prepared_collection.collection_name
        if in_memory_config is None:
            in_memory_config = self._default_memory_config(collection_name=collection_name, project_name=project_name, project_path=project_path)
        constructed = self.__class__.model_construct(provider=provider, tag=provider.variable, connection=connection, client_options=prepared_client_options, collection=prepared_collection, batch_size=batch_size, in_memory_config=in_memory_config)
        for field_name in constructed.model_fields:
            object.__setattr__(self, field_name, getattr(constructed, field_name))
        object.__setattr__(self, '__pydantic_fields_set__', set(constructed.model_fields.keys()))

    @staticmethod
    def _get_persist_path(*, collection_name: str | None=None, project_name: str | None=None, project_path: Path | None=None) -> Path:
        """Get the persist path from in_memory_config."""
        return Path(f'{get_user_state_dir()}/vectors/{generate_collection_name(project_name=project_name, project_path=project_path)}')

    @staticmethod
    def _default_memory_config(collection_name: str | None=None, project_name: str | None=None, project_path: Path | None=None, persist_path: Path | None=None) -> MemoryConfig:
        """Return the default memory config."""
        return MemoryConfig(auto_persist=True, persist_interval=300, persist_path=persist_path or MemoryVectorStoreProviderSettings._get_persist_path(collection_name=collection_name, project_name=project_name, project_path=project_path))
...
def _is_cloud_provider(_instance: BaseProviderSettings) -> bool:
    if _instance.provider.always_cloud or not _instance.provider.always_local:
        return True
    return bool(_instance.client_options and (url := getattr(_instance.client_options, 'url', getattr(_instance.client_options, 'endpoint', getattr(_instance.client_options, 'base_url', None)))) is not None and (not is_local_host(url)))
...
class EmbeddingProviderSettings(BaseEmbeddingProviderSettings):
    ...
    model_name: Annotated[ModelNameT, Field(description='The name of the embedding model to use. This should correspond to a model supported by the selected provider and formatted as the provider expects. For builtin models, this is the name as listed with `codeweaver list models`.')]
    embedding_config: Annotated[EmbeddingConfigT, Field(description='Model configuration for embedding operations.')]
    client_options: Annotated[GeneralEmbeddingClientOptionsType | None, Field(description="Client options for the provider's client.", discriminator='tag')] = None
    ...
    def client(self) -> LiteralSDKClient:
        ...
        if self.provider not in (Provider.AZURE, Provider.HEROKU):
            raise ValueError(f'Cannot resolve embedding client for provider {self.provider.variable}.')
...
class AzureEmbeddingProviderSettings(AzureProviderMixin, EmbeddingProviderSettings):
    ...
    client_options: Annotated[Annotated[CohereClientOptions, Tag(Provider.COHERE.variable)] | Annotated[OpenAIClientOptions, Tag(Provider.OPENAI.variable)], Field(description="Client options for the provider's client.", discriminator=Discriminator(discriminate_azure_embedding_client_options))] | None = None

    @model_validator(mode='after')
    def _validate_client_options(self) -> Self:
        """Validate and adjust client options for Azure embedding providers."""
        if self.client_options and self.client_options.base_url and (self.api_key or self.client_options.api_key):
            return self
        if not self.client_options:
            client_options = CohereClientOptions() if self.client == SDKClient.COHERE else OpenAIClientOptions()
        else:
        ...
        is_cohere = isinstance(client_options, CohereClientOptions) or self.client == SDKClient.COHERE
        if not options.get('base_url') and (endpoint := try_for_azure_endpoint(options, cohere=is_cohere)):
            options['base_url'] = AnyUrl(endpoint)
        final_client_options = {k: v for k, v in options.items() if v is not None and k not in {'model_deployment', 'endpoint', 'region_name'}}
        client = CohereClientOptions(**final_client_options) if is_cohere else OpenAIClientOptions(**final_client_options)
        object.__setattr__(self, 'client_options', client)
        for k, v in {key: value for key, value in options.items() if key in {'model_deployment', 'endpoint', 'region_name', 'api_key'}}.items():
            if v and (not hasattr(self, k) or (value := getattr(self, k, None)) is None or (value and value != v)):
                setattr(self, k, v)
        return self
...
class BedrockEmbeddingProviderSettings(BedrockProviderMixin, EmbeddingProviderSettings):
    ...
    client_options: Annotated[BedrockClientOptions | None, Field(description="Client options for the provider's client.")] = None
...
class FastEmbedEmbeddingProviderSettings(FastEmbedProviderMixin, EmbeddingProviderSettings):
    ...
    client_options: Annotated[FastEmbedClientOptions | None, Field(description="Client options for the provider's client.")] = None
...
class SparseEmbeddingProviderSettings(BaseProviderSettings):
    ...
    model_name: Annotated[ModelNameT, Field(description='The name of the sparse embedding model to use.')]
    sparse_embedding_config: Annotated[SparseEmbeddingConfigT, Field(description='Model configuration for sparse embedding operations.')]
    client_options: Annotated[SentenceTransformersClientOptions | None, Field(description="Client options for the provider's client.")] = None
...
class FastEmbedSparseEmbeddingProviderSettings(FastEmbedProviderMixin, SparseEmbeddingProviderSettings):
...
class RerankingProviderSettings(BaseProviderSettings):
    ...
    reranking_config: Annotated[RerankingConfigT, Field(description='Model configuration for reranking operations.')]
    top_n: PositiveInt | None = None
    client_options: Annotated[GeneralRerankingClientOptionsType, Field(description="Client options for the provider's client.")] | None = None
...
class FastEmbedRerankingProviderSettings(FastEmbedProviderMixin, RerankingProviderSettings):
    ...
    client_options: Annotated[FastEmbedClientOptions | None, Field(description="Client options for the SDK provider's client.")] = None
    tag: Literal['fastembed'] = 'fastembed'
...
class BedrockRerankingProviderSettings(BedrockProviderMixin, RerankingProviderSettings):
    ...
    client_options: Annotated[BedrockClientOptions | None, Field(description="Client options for the SDK provider's client.")] = None
    tag: Literal['bedrock'] = 'bedrock'

class DataProviderSettings(BaseProviderSettings):
    """Settings for data providers."""
    other: Annotated[dict[str, Any] | None, Field(description='Other provider-specific settings.')] = None
    ...
    def is_cloud(self) -> bool:
        """Return True if the provider is a cloud provider, False otherwise."""
        return _is_cloud_provider(self)
type ModelString = Annotated[str, Field(description='The model string, as it appears in `pydantic_ai.models.KnownModelName`.')]

class AgentProviderSettings(BaseProviderSettings):
    """Settings for agent models."""
    model: ModelString
    model_options: SkipValidation[Any]
...
__all__ = ('AgentProviderSettings', 'AzureEmbeddingProviderSettings', 'AzureProviderMixin', 'BaseEmbeddingProviderSettings', 'BaseProviderSettings', 'BaseProviderSettingsDict', 'BedrockEmbeddingProviderSettings', 'BedrockProviderMixin', 'BedrockRerankingProviderSettings', 'CollectionConfig', 'ConnectionConfiguration', 'ConnectionRateLimitConfig', 'DataProviderSettings', 'EmbeddingProviderSettings', 'FastEmbedEmbeddingProviderSettings', 'FastEmbedProviderMixin', 'FastEmbedRerankingProviderSettings', 'FastEmbedSparseEmbeddingProviderSettings', 'MemoryConfig', 'MemoryVectorStoreProviderSettings', 'ModelString', 'QdrantVectorStoreProviderSettings', 'RerankingProviderSettings', 'SparseEmbeddingProviderSettings', 'VectorStoreProviderSettings')

...
from qdrant_client.http.models.models import BinaryQuantization, CollectionInfo, CollectionsResponse, Datatype, Distance, Document, Modifier, PointStruct, QueryResponse, SparseIndexParams, SparseVector, SparseVectorParams, VectorParams
from codeweaver.core import CodeChunk, DictView, Provider, ProviderError, SearchResult, SearchStrategy, StrategizedQuery, TypeIs
...
class QdrantBaseProvider(VectorStoreProvider[AsyncQdrantClient], ABC):
    ...
    async def _metadata(self) -> CollectionMetadata | None:
        ...
        collection = await self.get_collection(collection_name=self._collection)
        if collection and collection.config and hasattr(collection.config, 'metadata') and collection.config.metadata:
    ...
    def _generate_metadata(self, *, for_backup: bool=False, collection_name: str | None=None) -> CollectionMetadata:
        ...
        sparse_model = getattr(self._embedding_caps['backup_sparse'], 'name', None) if for_backup else self.sparse_model
        sparse_model = sparse_model or 'qdrant/bm25'
        return CollectionMetadata.model_validate({'provider': self._provider.variable, 'created_at': datetime.now(UTC), 'project_name': project_name, 'vector_config': {'dense': VectorParams(size=getattr(self._embedding_caps['backup_dense'], 'default_dimension', 256) if for_backup else self._settings.get('dense', {}).get('dimension') or dense_dim if (dense_dim := getattr(self._embedding_caps.get('dense'), 'default_dimension', None)) is not None else 768, distance=distance, quantization_config=quantization_config, datatype=datatype)}, 'sparse_config': {'sparse': SparseVectorParams(index=SparseIndexParams(datatype=Datatype.FLOAT16), modifier=Modifier.IDF if sparse_model == 'qdrant/bm25' else None)}, 'collection_name': collection_name or self.config.get('collection_name') or self._default_collection_name(), 'is_backup': for_backup, 'dense_model': getattr(self._embedding_caps['backup_dense'], 'name', None) if for_backup else self.dense_model, 'sparse_model': sparse_model})
    ...
    async def _initialize(self) -> None:
        ...
        if self._client is not None:
            logger.debug('%s already initialized, skipping re-initialization', self.__class__.__name__)
            return
    ...
    async def search(self, vector: StrategizedQuery | MixedQueryInput, query_filter: Filter | None=None) -> list[SearchResult]:
        ...
        await self._ensure_collection(collection_name=self._collection or self._default_collection_name())
    ...
    async def _ensure_collection(self, collection_name: str, dense_dim: int | None=None, *, for_backup: bool=False) -> None:
        ...
        try:
            if await self._client.collection_exists(collection_name):
                ...
                return
            await self._client.create_collection(**self._generate_metadata(for_backup=for_backup, collection_name=collection_name).to_collection())
            self._known_collections.add(collection_name)
    ...
    async def _validate_collection_config(self, collection_name: str) -> None:
        ...
        def _raise_dimension_error() -> NoReturn:
            ...
            raise DimensionMismatchError(dedent(f"                Collection '{collection_name}' has {actual_dense.size}-dimensional vectors but current configuration specifies {expected_dense.size} dimensions.\n\n                This typically happens when:\n\n                    • The embedding model changed (e.g., switched providers or model versions)\n                    • The embedding configuration changed\n                    • The collection was created with different settings\n                "), details={'collection': collection_name, 'actual_dimension': actual_dense.size, 'expected_dimension': expected_dense.size, 'resolution_command': 'cw index --clear'}, suggestions=['  1. Rebuild the collection: cw index --clear', '  2. Or revert to the embedding model and settings that created this collection'])
        try:
            ...
            if isinstance(actual_vectors, dict) and 'dense' in actual_vectors:
                ...
                if actual_dense.distance != expected_dense.distance:
                    logger.warning("Collection '%s' uses %s distance metric but current configuration specifies %s. Search results may differ from expectations. Consider rebuilding: cw index --clear", collection_name, actual_dense.distance.value, expected_dense.distance.value)
                if hasattr(actual_dense, 'datatype') and hasattr(expected_dense, 'datatype') and (actual_dense.datatype != expected_dense.datatype):
                    logger.warning("Collection '%s' datatype mismatch: %s (actual) vs %s (expected). This may affect storage efficiency and precision.", collection_name, actual_dense.datatype.value, expected_dense.datatype.value)
        except Exception as e:
            logger.debug("Could not validate collection config for '%s'", collection_name, exc_info=e)

    async def create_payload_index(self, collection_name: str, field_name: str, field_schema: Literal['keyword', 'integer', 'float', 'geo', 'text', 'datetime', 'bool', 'uuid']) -> CollectionsResponse:
        ...
        try:
            return await self._client.create_payload_index(collection_name=collection_name, field_name=field_name, field_schema=field_schema)
        except Exception as e:
            logger.warning("Failed to create payload index on '%s' in collection '%s'", field_name, collection_name, extra={'error': str(e)})
            raise ProviderError(f"Failed to create payload index on '{field_name}': {e}") from e

    async def _execute_search_query(self, vector: StrategizedQuery, collection_name: str) -> list[Any] | Any:
        ...
        if vector.is_hybrid():
            ...
            if isinstance(prefetch, list) and prefetch:
                for i, p in enumerate(prefetch):
                    logger.info('Prefetch[%d]: query type=%s, using=%s', i, type(p.query), getattr(p, 'using', None))
            response = await self._client.query_points(**query_params)
        else:
            response = await self._client.query_points(**vector.to_query(kwargs=args | {'collection_name': collection_name}))
        return response.points if hasattr(response, 'points') else response

    def _normalize_vector_input(self, vector: StrategizedQuery | MixedQueryInput) -> StrategizedQuery:
        ...
        if isinstance(vector, dict):
            if 'indices' in vector and 'values' in vector:
                sparse = SparseEmbedding(indices=vector['indices'], values=[float(x) if isinstance(x, int) else x for x in vector['values']])
            elif 'sparse' in vector:
                sparse = SparseEmbedding(indices=vector['sparse'].get('indices', []), values=[float(x) if isinstance(x, int) else x for x in vector['sparse'].get('values', [])])
            if 'dense' in vector:
        ...
        strategy = SearchStrategy.HYBRID_SEARCH if dense and sparse else SearchStrategy.DENSE_ONLY if dense else SearchStrategy.SPARSE_ONLY
    ...
    def _convert_search_results(self, results: Any, vector: StrategizedQuery) -> list[SearchResult]:
        ...
        for point in points:
            ...
            search_result = SearchResult.model_construct(content=payload.chunk, file_path=Path(payload.file_path) if payload.file_path else None, score=point.score, metadata={'query': vector.query, 'strategy': vector.strategy})
    ...
    def _prepare_sparse_vector_data(self, sparse_embedding: CodeWeaverSparseEmbedding, sparse_vector: SparseVector, vectors: dict[str, Any]):
        indices = sparse_embedding.indices
        values = sparse_embedding.values
        normed_indices = indices if isinstance(indices, list) else list(indices) if isinstance(indices, Iterable) else [indices]
        normed_values = values if isinstance(values, list) else list(values) if isinstance(values, Iterable) else [values]
        vectors['sparse'] = sparse_vector(indices=normed_indices, values=normed_values)
    ...
    def _create_payload(self, chunk: CodeChunk, *, for_backup: bool=False) -> HybridVectorPayload:
        ...
        """
        return HybridVectorPayload(chunk=chunk, chunk_id=chunk.chunk_id.hex, chunked_on=datetime.fromtimestamp(chunk.timestamp).astimezone(UTC).isoformat(), file_path=str(chunk.file_path) if chunk.file_path else '', line_start=chunk.line_range.start, line_end=chunk.line_range.end, indexed_at=datetime.now(UTC).isoformat(), hash=chunk.blake_hash, provider=self._provider.variable, is_backup=for_backup, embedding_complete=bool(chunk.dense_batch_key and chunk.sparse_batch_key))
    ...
    async def delete_collection(self, collection_name: str | None=None) -> bool:
        ...
        try:
            ...
            raise ProviderError(f"Failed to delete collection '{target_collection}': {e}", details={'collection': target_collection, 'error': str(e)}) from e
        else:
            return True
    ...
    async def delete_by_file(self, file_path: Path) -> None:
        ...
        _ = await self._client.delete(collection_name=collection_name, points_selector=QdrantFilter(must=[FieldCondition(key='file_path', match=MatchValue(value=str(file_path)))]))
    ...
    async def get_collection(self, collection_name: str) -> CollectionInfo:
        ...
        try:
            ...
            raise ProviderError(f"Failed to get collection '{collection_name}': {e}", details={'collection': collection_name, 'error': str(e)}) from e
        else:
    ...
    async def delete_by_name(self, names: list[str]) -> None:
        ...
        from qdrant_client.models import Filter as QdrantFilter
        _ = await self._client.delete(collection_name=collection_name, points_selector=QdrantFilter(must=[FieldCondition(key='chunk.chunk_name', match=MatchAny(any=names))]))
        await self.handle_persistence()

    def _chunks_to_points(self, chunks: list[CodeChunk], *, for_backup: bool=False) -> list[PointStruct]:
        ...
        for chunk in chunks:
            ...
            points.append(PointStruct(id=chunk.chunk_id.hex, vector=vectors, payload=serialized_payload))
        return points
    ...
    async def upsert(self, chunks: list[CodeChunk], *, for_backup: bool=False) -> None:
        ...
        if not chunks:
            return
        self._ensure_client(self._client)
        await self._ensure_collection(collection_name=self._collection or self._default_collection_name(), for_backup=for_backup)
        if not self._client:
            raise ProviderError('Qdrant client not initialized')
        collection_name = self.collection
        if not collection_name:

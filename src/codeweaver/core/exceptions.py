...
def _get_issue_information() -> tuple[str, ...]:
    ...
    if _is_tty():
        return ('[dark orange]CodeWeaver[/dark orange] [bold magenta]is in alpha[/bold magenta]. Please report possible bugs at https://github.com/knitli/codeweaver/issues', '', "If you're not sure something is a bug, you can open a discussion at: https://github.com/knitli/codeweaver/discussions", '', '[bold]Thank you for helping us improve CodeWeaver! ❤️[/bold]')
    return ('CodeWeaver is in alpha. Please report possible bugs at https://github.com/knitli/codeweaver/issues', '', "If you're not sure something is a bug, you can open a discussion at: https://github.com/knitli/codeweaver/discussions", '', 'Thank you for helping us improve CodeWeaver!')

def _get_reporting_info(detail_parts: list[str]) -> str:
    """Generate issue reporting information."""
    detail_parts = detail_parts or []
    return '\n'.join(('Include the following information when reporting issues:', '- Details: ' + ', '.join(detail_parts) if detail_parts else '- No additional details provided.', ''))
...
class CodeWeaverError(Exception):
    ...
    def __init__(self, message: str, *, details: dict[str, Any] | None=None, suggestions: list[str] | None=None, location: LocationInfo | None=None) -> None:
    ...
    def __str__(self) -> str:
        ...
        if self.details:
            ...
            detail_parts.extend((f"{key.replace('_', ' ')}: {self.details[key]}" for key in ['actual_depth', 'max_depth', 'actual_tokens', 'max_tokens', 'chunk_count', 'max_chunks', 'timeout_seconds', 'elapsed_seconds', 'line_number'] if key in self.details))
            if detail_parts:
...
class MissingValueError(CodeWeaverError):
    ...
    def __init__(self, msg: str | None, field: str, details: dict[str, Any] | None=None, suggestions: list[str] | None=None) -> None:
        """Initialize MissingValueError.

        Args:
            field: The name of the missing field
        """
        super().__init__(message=msg or f'Missing value for field: {field}', details=details, suggestions=suggestions)
        self.field = field
...
class DependencyNotAvailableError(CodeWeaverError):
    ...
    def __init__(self, msg: str | None, dependency_name: str, required_package: str, details: dict[str, Any] | None=None, suggestions: list[str] | None=None) -> None:
        ...
        """
        super().__init__(message=msg or f"Required dependency '{dependency_name}' from package '{required_package}' is not available. Please install the package to proceed.", details=details, suggestions=suggestions)
...
class CircularDependencyError(DependencyInjectionError):
    ...
    def __init__(self, cycle: str, details: dict[str, Any] | None=None, suggestions: list[str] | None=None) -> None:
        ...
        Args:
            ...
            suggestions: Actionable suggestions for resolving the error
        """
        super().__init__(message=f'Circular dependency detected: {cycle}', details=details, suggestions=suggestions or ['Review dependency chain and break the cycle by:', '  1. Using property injection instead of constructor injection', '  2. Introducing an interface/abstraction to break the cycle', '  3. Restructuring components to remove the circular reference'])
        self.cycle = cycle
...
class UnresolvableDependencyError(DependencyInjectionError):
    ...
    def __init__(self, interface: type[Any], reason: str, details: dict[str, Any] | None=None, suggestions: list[str] | None=None) -> None:
        ...
        Args:
            ...
            suggestions: Actionable suggestions for resolving the error
        """
        super().__init__(message=f'Cannot resolve dependency {interface.__name__}: {reason}', details=details, suggestions=suggestions or [f'Ensure {interface.__name__} is registered with the DI container:', f'  container.register({interface.__name__}, factory_function)', 'Or use the dependency_provider decorator:', f'  dependency_provider({interface.__name__})', f'  def get_{interface.__name__.lower()}(): ...'])
...
class ScopeViolationError(DependencyInjectionError):
    ...
    def __init__(self, dependent: str, dependency: str, reason: str, details: dict[str, Any] | None=None, suggestions: list[str] | None=None) -> None:
        ...
        Args:
            ...
            suggestions: Actionable suggestions for resolving the error
        """
        super().__init__(message=f'Scope violation: {dependent} cannot depend on {dependency}: {reason}', details=details, suggestions=suggestions or ['Ensure scope hierarchy is correct:', '  singleton > request > function', 'A dependency can only depend on scopes of equal or longer lifetime.'])
...
class DependencyResolutionError(DependencyInjectionError):
    ...
    def __init__(self, errors: list[DependencyInjectionError], details: dict[str, Any] | None=None) -> None:
        ...
        super().__init__(message=f'Multiple dependency resolution errors:\n{error_messages}', details=details, suggestions=['Fix each error listed above.', 'Errors are listed in the order they were encountered.'])
        self.errors = errors
__all__ = ('CircularDependencyError', 'CodeWeaverError', 'CollectionNotFoundError', 'ConfigurationError', 'DependencyInjectionError', 'DependencyNotAvailableError', 'DependencyResolutionError', 'DimensionMismatchError', 'IndexingError', 'InitializationError', 'InvalidEmbeddingModelError', 'MissingValueError', 'PersistenceError', 'ProviderError', 'ProviderSwitchError', 'QueryError', 'RerankingProviderError', 'ScopeViolationError', 'UnresolvableDependencyError', 'ValidationError')
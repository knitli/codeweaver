...
INTROSPECTION_ATTRS: frozenset[str] = frozenset({'__annotations__', '__class__', '__closure__', '__code__', '__defaults__', '__dict__', '__doc__', '__func__', '__globals__', '__kwdefaults__', '__module__', '__name__', '__qualname__', '__self__', '__signature__', '__text_signature__', '__wrapped__'})
    ...
    def __getattr__(self, name: str) -> LazyImport[Import]:
        ...
        if name in INTROSPECTION_ATTRS:
            try:
                ...
                raise AttributeError(f'Attribute {name!r} not found in resolved object {resolved!r}') from e
            else:
...
def create_lazy_getattr(dynamic_imports: MappingProxyType[str, tuple[str, str]], module_globals: dict[str, object], module_name: str) -> object:

...
from typing import Annotated, Any, NamedTuple, Self, TypeGuard, cast, override

from pydantic import UUID7, Field, NonNegativeInt, PositiveInt, computed_field, model_validator

from codeweaver.core.constants import ONE_LINE
...
class Span(NamedTuple):
    ...
    # Explicit definition removed for Python 3.13 compatibility
    ...
    def __str__(self) -> str:
        ...
        return f"lines {self.start}-{self.end} (source: {self.source_id})"

    def __repr__(self) -> str:
        """Return a string representation of the span."""
        return f"Span({self.start}, {self.end}, {self.source_id})"
    ...
    def __and__(self, other: Span) -> Span | None:  # Intersection
        ...
        return Span(start=start, end=end, source_id=self.source_id) if start <= end else None
    ...
    def __sub__(self, other: Span) -> Span | tuple[Span, Span] | None:  # Difference
        ...
        if other.start > self.start and other.end < self.end:
            return (
                Span(start=self.start, end=other.start - ONE_LINE, source_id=self.source_id),
                Span(start=other.end + ONE_LINE, end=self.end, source_id=self.source_id),
            )
    ...
    def __call__(cls, span: SpanTuple | Span) -> Span:
        ...
        return cls(start=start, end=end, source_id=source_id or uuid7())
    ...
    def from_sourced_lines(self, start: NonNegativeInt, end: NonNegativeInt) -> Span:
        ...
        return Span(start=start, end=end, source_id=self.source_id)
    ...
    def intersection(self, other: Span) -> Span | None:
        ...
        return None if self.source_id != other.source_id else self & other
...
class SpanGroup(BasedModel):
    ...
    def __or__(self, other: Self) -> SpanGroup:  # Union
        ...
        return SpanGroup(spans=self.spans | other.spans)
    ...
    def __and__(self, other: Self) -> SpanGroup:  # Intersection
        ...
        return SpanGroup(spans={s for s in intersected if s})

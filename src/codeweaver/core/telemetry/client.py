...
class TelemetryService:
    ...
    def __init__(self, api_key: SecretStr | str | None=None, host: str='https://us.i.posthog.com', *, enabled: bool=True):
        """
        ...
        import os
        if api_key is None:
            api_key = os.environ.get('CODEWEAVER_POSTHOG_API_KEY') or os.environ.get('CODEWEAVER__TELEMETRY__API_KEY') or os.environ.get('CODEWEAVER__TELEMETRY__POSTHOG_PROJECT_KEY') or CODEWEAVER_POSTHOG_PROJECT_KEY
        ...
        if self.enabled and api_key:
            try:
                self._client = Posthog(project_api_key=api_key, host=host, debug=False, disable_geoip=True)
                self.logger.info('PostHog telemetry client initialized')
            except ImportError:
                self.logger.warning("PostHog package not installed, telemetry disabled. Install with: uv pip install 'code-weaver[recommended]'")
                self.enabled = False
                self._client = None
            except Exception:
    ...
    def from_settings(cls, settings: TelemetrySettingsDep) -> TelemetryService:
        ...
        if not settings.enabled or not settings.posthog_project_key or (not settings.posthog_host) or settings.disable_telemetry:
            return cls(enabled=False)
        posthog_host = HttpUrl('https://us.i.posthog.com') if isinstance(settings.posthog_host, Unset) else settings.posthog_host
        return cls(api_key=settings.posthog_project_key.get_secret_value(), host=posthog_host.encoded_string(), enabled=not settings.disable_telemetry)
    ...
    def capture(self, event: str, properties: dict[str, Any], *, distinct_id: UUID7HexT | None=None) -> None:
        """
        ...
        try:
            ...
            _ = self._client.capture(distinct_id=actual_distinct_id, event=event, properties=properties)
    ...
    def capture_with_serialization(self, event: str, data_obj: Any, *, distinct_id: UUID7HexT | None=None) -> None:
        """
        ...
        try:
            if hasattr(data_obj, 'serialize_for_telemetry'):
                ...
                self.logger.warning("Object %s does not have serialize_for_telemetry method, skipping event '%s'", type(data_obj).__name__, event)
        except Exception:

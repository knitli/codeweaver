...
class FailoverSettings(BasedModel):
    ...
    CodeWeaver creates this secondary backup automatically when your system is idle and keeps it up to date at regular intervals. If one of your primary embedding or vector store providers become unreachable, CodeWeaver will automatically switch to the backup store to maintain service continuity. When the primary store becomes reachable again, CodeWeaver can automatically restore it as the primary store after a short delay. (We switch on embedding failures because we don't want codeweaver to *ever* become stale -- even if the vector store is reachable, if we can't get new embeddings then the system can't learn new information.)
    ...
    Importantly, while the system is lightweight, it's still extremely capable -- our backup is more robust than most search tools' primary systems. It maintains full hybrid multivector search and reranking support, ensuring that your search experience remains seamless even during failover events.
    """
    ...
    backup_sync_interval: Annotated[
        int, Field(description="Interval in seconds for syncing primary state to backup")
    ] = 300
    ...
    restore_delay: Annotated[
        int, Field(description="Seconds to wait after primary becomes healthy before restoring")
    ] = 60
...
DefaultFailoverSettings = FailoverSettings().model_dump()

__all__ = ("DefaultFailoverSettings", "FailoverSettings")

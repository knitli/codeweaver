...
```python
get_stdio_server(config_file=None, project_path=None, host=None, port=None)
```
...
```python
run(config_file=None, project_path=None, host=None, port=None, transport="stdio", verbose=False, debug=False)
```

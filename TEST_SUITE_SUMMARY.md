...
# Run with coverage
python -m pytest tests/unit/providers/config/test_asymmetric_config.py --cov=codeweaver.providers.config.kinds --cov-report=term-missing
...
1. **Agent A** completes AsymmetricEmbeddingConfig implementation in `src/codeweaver/providers/config/kinds.py`
...
- `AsymmetricEmbeddingConfig` class in `codeweaver.providers.config.kinds`

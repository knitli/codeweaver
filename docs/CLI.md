...
- [`start`](#codeweaver-start)
- [`stop`](#codeweaver-stop)
...
* `start`: Start CodeWeaver background services.
* `status`: Show CodeWeaver runtime status.
* `stop`: Stop CodeWeaver background services.
...
* `-c, --config-file`: Path to CodeWeaver configuration file, only needed if not using defaults.
* `-p, --project`: Path to project directory. CodeWeaver will attempt to auto-detect if not provided.
...
* `--display.console.highlighter`: Default highlighter.  *[default: <rich.highlighter.ReprHighlighter object at 0x711e794134d0>]*

...
def _languages() -> list[str]:
    ...
    return sorted(all_languages | {f'{lang} (AST support)' for lang in SemanticSearchLanguage} | {lang.variable for lang in ConfigLanguage if not lang.is_semantic_search_language})

def embedding_providers() -> list[Provider]:
    """Get a list of embedding providers."""
    return [provider for provider in Provider if provider.is_embedding_provider()]

def reranking_providers() -> list[Provider]:
    """Get a list of reranking providers."""
    return [provider for provider in Provider if provider.is_reranking_provider()]
...
def generate_server_yaml() -> dict:
    ...
    parameters_props = {}
    parameters_required = []
    important_vars = ['CODEWEAVER_PROJECT_PATH', 'CODEWEAVER_EMBEDDING_PROVIDER', 'CODEWEAVER_EMBEDDING_MODEL', 'CODEWEAVER_EMBEDDING_API_KEY', 'CODEWEAVER_VECTOR_STORE_PROVIDER', 'CODEWEAVER_VECTOR_STORE_URL', 'CODEWEAVER_VECTOR_STORE_API_KEY', 'CODEWEAVER_RERANKING_PROVIDER', 'CODEWEAVER_RERANKING_MODEL', 'CODEWEAVER_RERANKING_API_KEY']
    for var_name in important_vars:
        ...
        if var_info.is_secret:
            secrets.append({'name': f'codeweaver.{param_name}', 'env': env_name, 'example': f'<{env_name}>'})
            parameters_props[param_name] = {'type': 'string'}
        else:
            example = var_info.default or f'<{env_name}>'
            regular_env.append({'name': env_name, 'example': example, 'value': f'{{{{codeweaver.{param_name}}}}}'})
            param_type = 'string'
    ...
    return {'name': 'codeweaver', 'image': 'mcp/codeweaver', 'type': 'server', 'meta': {'category': 'developer-tools', 'tags': ['code-search', 'semantic-search', 'embeddings', 'ai', 'developer-tools', 'ast', 'hybrid-search', 'rag']}, 'about': {'title': 'CodeWeaver - Code Search for AI Agents', 'description': f'Semantic code search for AI agents. Hybrid search across {len(_languages())} languages with {len(embedding_providers())} embedding providers.\n\n            Works offline with local models or cloud providers (Voyage, OpenAI, Cohere, etc.). AST-aware parsing, language-aware chunking, automatic failover.\n\n            One simple MCP tool for any client.', 'icon': 'https://github.com/knitli/codeweaver/raw/refs/heads/main/docs/assets/codeweaver-favico.png'}, 'source': {'project': 'https://github.com/knitli/codeweaver', 'commit': get_git_commit()}, 'config': {'description': f'Configure CodeWeaver with your project path, optional choice of providers and models, and optional vector store settings. Supports {len(embedding_providers())} embedding providers, {len(sparse_embedding_providers())} sparse embedding providers, {len(reranking_providers())} reranking providers, and {len(_languages())} programming languages.', 'secrets': secrets or None, 'env': regular_env or None, 'parameters': {'type': 'object', 'properties': parameters_props, 'required': parameters_required or None} if parameters_props else None}}
...
def save_server_yaml(config: dict | None=None) -> Path:
    ...
    with file_path.open('w') as f:
        yaml.dump(cleaned_config, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
    return file_path

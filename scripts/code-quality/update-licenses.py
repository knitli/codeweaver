...
EXTENSION_COMMENT_STYLE_MAP = EXTENSION_COMMENT_STYLE_MAP | {'.astro': TSXCommentStyle, '.pkl': CppCommentStyle, '.jsonc': CppCommentStyle}
...
CONTRIBUTORS = Parameter(consume_multiple=True, help='Name and email of the contributor(s) to add. May be provided multiple times, or as a json list.', negative=(), json_list=True)
INTERACTIVE = Parameter(negative=(), help='Run the script in interactive mode, prompting for contributors.')
app = App(name='Knitli License Updater', version=__version__, help='Update licenses for files in the repository using Reuse. Respects .gitignore.', help_format='rich', default_parameter=Parameter(negative=()), help_on_error=True)

def is_right_pkl(path: Path) -> bool:
    """Check if a .pkl file is binary (python pkl) or if it's a config file."""
    if path.suffix != '.pkl':
        return False
    if path.name in ('hk.pkl', 'hk.local.pkl', 'fileGroups.pkl', 'helpers.pkl', 'toolchain.pkl', 'workspace.pkl', 'tasks.pkl'):
        return True
    try:
        ...
        return not bool(content.translate(None, bytearray({7, 8, 9, 10, 12, 13, 27} | set(range(32, 256)) - {127})))
...
BASE_CMD = ['reuse', 'annotate', '--year', years(), '--copyright', 'Knitli Inc. <knitli@knit.li>', '--merge-copyrights', '--fallback-dot-license', '--skip-existing']
...
def get_files_with_missing() -> list[Path] | None:
    ...
    try:
        ...
        if (non_compliant_report := output.get('non_compliant', {})) and (missing_files := (non_compliant_report.get('missing_copyright_info', []) + non_compliant_report.get('missing_licensing_info', []))):
...
def pre_commit() -> bool:
    ...
    try:
        if not (staged_files := get_staged_files()):
            ...
            return True
        user_name_result = subprocess.run([git_path, 'config', '--get', 'user.name'], capture_output=True, text=True, check=True)
        user_email_result = subprocess.run([git_path, 'config', '--get', 'user.email'], capture_output=True, text=True, check=True)
        contributor = f'{user_name_result.stdout.strip()} <{user_email_result.stdout.strip()}>'
        cmds = [[REUSE_PATH, 'annotate', '--contributor', contributor, str(BASE_PATH / file)] for file in staged_files]
        with ThreadPoolExecutor() as executor:
            _ = executor.map(subprocess.run, cmds)
    except subprocess.CalledProcessError as e:
...
def get_contributor() -> str:
    ...
    if not sys.stdin.isatty():
        pre_commit()
    if (contributor := input("What's your name and email? (e.g. 'Adam Poulemanos <adam@knit.li>' (the default)): ").strip()):
...
def add(files: Annotated[list[Path], Parameter(validator=validators.Path(exists=True), required=True, consume_multiple=True, json_list=True)], *, contributors: Annotated[list[str], CONTRIBUTORS]=DEFAULT_CONTRIBUTORS, interactive: Annotated[bool, INTERACTIVE]=False) -> None:

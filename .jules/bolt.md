...
## 2026-04-12 - Walrus Operator Optimization
**Learning:** Using the walrus operator inside a list comprehension to avoid redundant execution of string methods (like `.strip()`) can be an effective micro-optimization. However, assignment-expression targets inside comprehensions are bound in the enclosing scope, so the assigned name will leak outside the comprehension and can shadow or overwrite an existing local or global variable if the same name is already in use.
**Action:** Prefer using the walrus operator `:=` in list comprehensions or conditionals when it meaningfully avoids repeated identical string manipulations (e.g., `.strip()`) or expensive evaluation calls and does not reduce readability or diverge from Python idioms.
